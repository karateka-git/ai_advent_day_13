...
Если несколько операций зависят друг от друга через общий state, файлы, persisted storage, общий runtime-контекст или результаты предыдущего шага, их нужно выполнять строго последовательно, если пользователь явно не попросил обратное.

Правило для Codex:
- параллелить только независимые операции;
- `setup -> verify`, `build -> installDist -> run`, миграции, smoke-check с общим storage и другие цепочки с общей зависимостью не запускать через параллельный wrapper;
- если есть сомнение, независимы ли шаги, считать их зависимыми и выполнять последовательно;
- если пользователь явно просит параллелить, сначала убедиться, что это не повредит общему state и не исказит результат проверки.
...
- перед первым прогоном в новой серии scripted smoke-check очищать старые артефакты в `build/smoke-check/`, чтобы результаты прошлых прогонов не смешивались с текущими;
- для этого в `scripts/run-scripted-session.ps1` использовать `-ClearSmokeArtifacts` на первом запуске серии;
