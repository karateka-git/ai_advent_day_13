...
interface Agent<T> {
    ...
    /**
     * Возвращает краткое user-facing пояснение о task-aware режиме ответа для следующего сообщения.
     *
     * Используется UI-слоем, чтобы показать, как текущая задача влияет на ответ агента, не раскрывая
     * внутренние детали prompt assembly.
     *
     * @param userPrompt текст нового пользовательского сообщения.
     * @return краткое пояснение или `null`, если специальное пояснение не требуется.
     */
    fun previewTaskBehavior(userPrompt: String): String?
